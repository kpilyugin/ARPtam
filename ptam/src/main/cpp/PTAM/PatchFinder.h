...
//test: might increase robustness in changing light situations
//#define COMPUTE_NCC 1
#define NCC_T 0.6
//ncc 0.5 in bad light?


class PatchFinder {
public:
    // Constructor defines size of search patch.
    PatchFinder(int nPatchSize = 8);//16 //16 too slow for BT200?
    ...
    void MakeTemplateCoarseCont(
            MapPoint& p); // If the warping matrix has already been pre-calced, use this.
    void MakeTemplateCoarse(MapPoint& p, SE3<> se3CFromW,
    ...
    void MakeTemplateCoarseNoWarp(KeyFrame& k, int nLevel,
                                  CVD::ImageRef irLevelPos); // Identity warp if no MapPoint struct exists yet.
    ...
    int ZMSSDAtPoint(CVD::BasicImage<CVD::byte>& im,
                     const CVD::ImageRef& ir); // This evaluates the score at one location
    float NCCAtPoint(CVD::BasicImage<CVD::byte>& im,
                     const CVD::ImageRef& ir); // This evaluates the score at one location
    // Results from step 3:
    ...
    // Step 5 Functions
    bool IterateSubPixToConvergence(KeyFrame& kf,
                                    int nMaxIts);  // Run inverse composition till convergence
    double IterateSubPix(
            KeyFrame& kf);     // Single iteration of IC. Returns sum-squared pixel update dist, or negative if out of imag
