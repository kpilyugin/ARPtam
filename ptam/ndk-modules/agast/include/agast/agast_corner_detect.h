...
//static AgastDetector7_12d detector; //not thread save!!!

//libcvd
inline void agast_corner_detect12d(const CVD::BasicImage<CVD::byte>& im, std::vector<CVD::ImageRef>& corners, int barrier)
{
	AgastDetector7_12d detector;
	...
	/*
	std::vector<CvPoint> nms_keypoints;
	detector.nms((unsigned char*)im.data(),keypoints,nms_keypoints);
...
namespace oast9_16{
//static OastDetector9_16 detector;//not thread save!!!
//libcvd
inline void oast_corner_detect16(const CVD::BasicImage<CVD::byte>& im, std::vector<CVD::ImageRef>& corners, int barrier)
{
	OastDetector9_16 detector;
