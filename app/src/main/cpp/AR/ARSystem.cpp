...
TrackingConfiguration ARSystem::trackingConfiguration() const {
    ...
    configuration.numberPointsForSructureOptimization = m_numberPointsForStructureOptimization;
...
void ARSystem::setTrackingConfiguration(const TrackingConfiguration &configuration) {
    ...
    m_numberPointsForStructureOptimization = configuration.numberPointsForSructureOptimization;
    ...
    m_trackerTransform.setMinMaxLevel(configuration.tracker_minImageLevel,
                                      configuration.tracker_maxImageLevel);
...
void ARSystem::setMapPointsDetectorConfiguration(
        const MapPointsDetectorConfiguration &configuration) {
    std::lock_guard<std::mutex> lock(m_mutex);
    (void) lock;
    m_candidatesDetector.setConfiguration(configuration);
}

MapInitializer ARSystem::initializer() const {
    std::lock_guard<std::mutex> lock(m_mutex);
    (void) lock;
...
Point2d ARSystem::resolution() const {
...
void ARSystem::setNumberIterationsForStructureOptimization(
...
void ARSystem::nextTrackingState() {
    ...
    switch (m_trackingState) {
        ...
        case TrackingState::CaptureSecondFrame: {
            ...
            MapInitializer::InitializationResult initializationResult = m_initializer.compute(
                    &m_map, &m_mapResourceManager, true);
...
void ARSystem::renderTrackingInfo() {
    std::lock_guard<std::mutex> lock(m_mutex);
    (void) lock;
    Point2d resolution = m_camera->imageSize();

    Point2f scale(0.75f * 2.0f / static_cast<float>(resolution.x),
                  2.0f / static_cast<float>(resolution.y));
    TVectorf blue = TVectorf::create(0, 0, 1);
    TVectorf green = TVectorf::create(0, 1, 0);
    TVectorf red = TVectorf::create(1, 0, 0);

    if (m_trackingState == TrackingState::CaptureSecondFrame) {
        ...
        std::cout << "Capturing second frame: number of features = "  <<
                firstFeatures.size() << ", " << secondFeatures.size() << std::endl;
        if (firstFeatures.empty() || secondFeatures.empty()) {
            std::cout << "No features! " << std::endl;
            return;
        }
        for (std::size_t i = 0; i < firstFeatures.size(); ++i) {
            const Point2f& firstF = firstFeatures[i];
            Point2f a(firstF.x * scale.x - 1.0f, firstF.y * scale.y - 1.0f);
            const Point2f& secondF = secondFeatures[i];
            Point2f b(secondF.x * scale.x - 1.0f, secondF.y * scale.y - 1.0f);
...
void ARSystem::process(const ImageRef<Rgba> &frame) {
    ...
    _process(m_blackWhiteFrame);
}


void ARSystem::process(const ImageRef<uchar> &frame) {
    _copyBlackWhiteFrame(frame);
    _process(m_blackWhiteFrame);
}

void ARSystem::_process(const ImageRef<uchar> &bwFrame) {
    std::lock_guard<std::mutex> lock(m_mutex);
    (void) lock;
    ...
    if (m_camera->imageSize() != bwFrame.size().cast<double>()) {
        Camera* camera = new Camera();
        camera->setImageSize(bwFrame.size());
        camera->setCameraParameters(m_cameraParameters);
    ...
    switch (m_trackingState) {
                ...
                if (m_trackingQuality == TrackingQuality::Good) {
                    if (needNewFrame) {
                        if ((int) m_map.countKeyFrames() > m_maxCountKeyFrames) {
                            m_map.deleteKeyFrame(m_map.getFurthestKeyFrame(&m_mapResourceManager,
                                                                           newFrame.worldPosition()));
                        }
        ...
        case TrackingState::CaptureSecondFrame: {
            ...
            m_initializer.setSecondFrame(m_camera, m_blackWhiteFrame);
            MapInitializer::InitializationResult initializationResult = m_initializer.compute(
                    &m_map, &m_mapResourceManager, false);
            ...
            switch (initializationResult) {
                case MapInitializer::InitializationResult::Success: {
                    ...
                    MapResourceLocker lockerR(&m_mapResourceManager, keyFrame.get());
                    (void) lockerR;
                ...
                case MapInitializer::InitializationResult::Reset: {
                    std::cout << "Initialization reset" << std::endl;
                    _reset();
                    break;
                }
            }
        ...
        case TrackingState::LostTracking: {
            bool foundNearestKeyFrame = false;
            {
                ...
                for (std::size_t i = 0; i < m_map.countKeyFrames(); ++i) {
                    ...
                    if (!k)
                        continue;
                    MapResourceLocker lockerR(&m_mapResourceManager, k.get());
                    (void) lockerR;
                    if (!k->isDeleted()) {
                ...
                if (keyFrame) {
                    ...
                    if (bestScore >
                        (int) (targetSmallImage.area() * ((255 * 0.18) * (255 * 0.18)))) {
                        keyFrame.reset();
                    }
                }
                if (keyFrame) {
...
void ARSystem::_converToBlackWhiteFrame(const ImageRef<Rgba> &frame) {
    ...
    for (int i = 0; i < area; ++i, ++bwPtr, ++rgbaPtr)
        *bwPtr = uchar((rgbaPtr->red + rgbaPtr->green + rgbaPtr->blue) / 3);
}

void ARSystem::_copyBlackWhiteFrame(const ImageRef<uchar> &frame) {
    if (frame.size() != m_blackWhiteFrame.size()) {
        m_blackWhiteFrame = Image<uchar>(frame.size());
    }
    const int area = m_blackWhiteFrame.area();
    uchar* dst = m_blackWhiteFrame.data();
    const uchar* src = frame.data();
    for (int i = 0; i < area; ++i, ++dst, ++src) {
        *dst = *src;
    }
}
...
void ARSystem::_optimizeMapPoints(PreviewFrame &frame) {
    ...
    for (; it != features.end();) {
        ...
        MapResourceLocker locker(&m_mapResourceManager, mapPoint.get());
        (void) locker;
        if (!mapPoint->isDeleted()) {
            mapPoint->optimize(&m_mapResourceManager, m_numberIterationsForStructureOptimization,
                               frame, it->positionOnFrame);
            ++countProcessedPoints;
            if (countProcessedPoints > m_numberPointsForStructureOptimization)
                break;
            ++it;
        } else {
...
std::shared_ptr<KeyFrame> ARSystem::_createNewKeyFrame(PreviewFrame &previewFrame) {
    ...
    MapResourceLocker lockerKeyFrame(&m_mapResourceManager, keyFrame.get());
    (void) lockerKeyFrame;

    std::vector<PreviewFrame::PreviewFeature> &features = previewFrame.previewFeatures();
    for (auto it = features.begin(); it != features.end(); ++it) {
        std::shared_ptr<MapPoint> mapPoint = it->mapPoint;
        MapResourceLocker lockerMapPoint(&m_mapResourceManager, mapPoint.get());
        (void) lockerMapPoint;
        if (!mapPoint->isDeleted()) {
