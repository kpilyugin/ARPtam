...
//In which feature level should the trails be computed during initialization
#define TRAIL_LEVEL 1
...
class Tracker {
public:
    Tracker(CVD::ImageRef irVideoSize, const ATANCamera& camera, Map& map, MapMaker& mm);
    ~Tracker();
...
protected:
    ...
    int SearchForPoints(std::vector<TrackerData*>& vTD,
                        ...
                        int nFineIts,
                        int start = 0,
                        int end = -1);  // Finds points in the image
    static void SearchForPointsThreaded(void*);
    static void* SearchForPointsThreaded2(void*);
    ...
    std::vector<TrackerData*> mAttempted;
...
public:
    bool useSensorDataForTracking;
    bool useSensorDataForInitialization;
protected:

#endif
    bool updateOnlyPosition;
    ...
    float mdTotalFracFound;
    float mdLargeFracFound;
