...
//ICG: some hacky try to speed up things, TODO check again and maybe remove?
#define USE_POINT_LIST 1
...
public:
    ...
    int AddCamera(SE3<> se3CamFromWorld,
                  bool bFixed); // Add a viewpoint. bFixed signifies that this one is not to be adjusted.
    ...
    int Compute(
            bool* pbAbortSignal);    // Perform bundle adjustment. Aborts if *pbAbortSignal gets set to true. Returns number of accepted update iterations, or negative on error.
    ...
    void AllocatePoints(int numPts);
    bool updatePositionOnly;

protected:

    inline void ProjectAndFindSquaredError(
            Meas& meas); // Project a single point in a single view, compare to measurement
...
#else
    std::vector<Meas> mMeasList;
    std::vector<Meas*> mMeasCurList;
    int nPoints;
#endif
...
#ifdef USE_POINT_LIST
typedef std::list<Meas>::iterator meas_b_it;
typedef std::list<Meas>::iterator meas_bc_it;
#else
typedef std::vector<Meas>::iterator meas_b_it;
typedef std::vector<Meas*>::iterator meas_bc_it;
#endif
