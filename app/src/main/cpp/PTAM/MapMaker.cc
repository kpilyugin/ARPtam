...
MapMaker::MapMaker(Map& m, const ATANCamera& cam)
    ...
    mdWiggleScale = 0.3;
//    GV3::Register(mgvdWiggleScale, "MapMaker.WiggleScale", 0.3,
...
void MapMaker::run() {
    ...
    while (!shouldStop())  // ShouldStop is a CVD::Thread func which return true if the thread is told to exit.
    {
        ...
        if (mbBundleConverged_Recent && mbBundleConverged_Full && rand() % 20 == 0 &&
            QueueSize() == 0) {
            //TIMER_START
...
bool MapMaker::InitFromStereo(KeyFrame& kF,
    ...
    *pkSecond = kS;
    ...
    double dNewTransMagn = sqrt(
            pkSecond->se3CfromW.get_translation() * pkSecond->se3CfromW.get_translation());
...
bool MapMaker::AddPointEpipolar(KeyFrame& kSrc,
    ...
    Vector<3> v3LineDirn_TC =
            kTarget.se3CfromW.get_rotation() * (kSrc.se3CfromW.get_rotation().inverse() * v3Ray_SC);
    ...
    Vector<3> v3CamCenter_TC =
            kTarget.se3CfromW * kSrc.se3CfromW.inverse().get_translation(); // The camera end
    Vector<3> v3RayStart_TC = v3CamCenter_TC + dStartDepth *
                                               v3LineDirn_TC;                               // the far-away end
    Vector<3> v3RayEnd_TC = v3CamCenter_TC + dEndDepth *
                                             v3LineDirn_TC;                               // the far-away end


    if (v3RayEnd_TC[2] <=
        v3RayStart_TC[2])  // it's highly unlikely that we'll manage to get anything out if we're facing backwards wrt the other camera's view-ray
        return false;
...
bool MapMaker::NeedNewKeyFrame(KeyFrame& kCurrent) {
    ...
    if (dTDist > 0.2) { //"MapMaker.MaxKFDistWiggleMult", 0.2, {
        //cout << "d:" << dDist;
        return true;
    }
    return false;
...
void MapMaker::BundleAdjustRecent() {
    ...
    std::unordered_set<KeyFrame*> sAdjustSet;
    ...
    std::unordered_set<MapPoint*> sMapPoints;
    for (std::unordered_set<KeyFrame*>::iterator iter = sAdjustSet.begin();
         iter != sAdjustSet.end();
         iter++) {
    ...
    std::unordered_set<KeyFrame*> sFixedSet;
...
void MapMaker::BundleAdjust(std::unordered_set<KeyFrame*> sAdjustSet,
                            std::unordered_set<KeyFrame*> sFixedSet,
                            std::unordered_set<MapPoint*> sMapPoints, bool bRecent) {
    TIMER_INIT
    TIMER_START
...
// Add the keyframes' poses to the bundle adjuster. Two parts: first nonfixed, then fixed.
    for (std::unordered_set<KeyFrame*>::iterator it = sAdjustSet.begin();
    ...
    for (std::unordered_set<KeyFrame*>::iterator it = sFixedSet.begin();
         it != sFixedSet.end(); it++) {
...
// Add the points' 3D position
    for (std::unordered_set<MapPoint*>::iterator it = sMapPoints.begin();
...
// Bundle adjustment did some updates, apply these to the map
    if (nAccepted > 0) {
        for (std::unordered_map<MapPoint*, int>::iterator itr = mPoint_BundleID.begin();
             itr != mPoint_BundleID.end(); itr++) {
            itr->first->v3WorldPos = b.GetPoint(itr->second);
        }

        for (std::unordered_map<KeyFrame*, int>::iterator itr = mView_BundleID.begin();
    ...
    while (!mqNewQueue.empty() && mvpKeyFrameQueue.size() == 0) {
        ...
        for (unsigned int i = 0; i < mMap.vpKeyFrames.size(); i++)
            if (ReFind_Common(*mMap.vpKeyFrames[i], *pNew)) {
                nFound++;
                updateDepthKF.insert(
                        mMap.vpKeyFrames[i]); //TODO: rethink this!!! update when removed??
            }
    }
...
void MapMaker::RefreshSceneDepth(KeyFrame* pKF) {
    ...
    pKF->dSceneDepthSigma = sqrt(
            (dSumDepthSquared / nMeas) - (pKF->dSceneDepthMean) * (pKF->dSceneDepthMean));
...
//
//    //save camera
//    map->InsertEndChild(mCamera.save(
//            helper)); //TODO allow different cameras for each keyframe, at the moment saving the camera is useless!!!
//
//    //save map
...
//    ATANCamera test((std::basic_string<char, char_traits<char>, allocator<char >> ()),
//    (CVD::ImageRef()));
