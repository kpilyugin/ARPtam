...
void MapPoint::RefreshPixelVectors() {
    ...
    v3PixelRight_W =
            k.se3CfromW.get_rotation().inverse() * (v3OneRightOnPlane_C - v3CenterOnPlane_C);
    v3PixelDown_W = k.se3CfromW.get_rotation().inverse() * (v3OneDownOnPlane_C - v3CenterOnPlane_C);
}

//XMLElement* MapPoint::save(MapSerializationHelper& helper) {
//    XMLDocument* doc = helper.GetXMLDocument();
//    XMLElement* mappoint = doc->NewElement("MapPoint");
//
//    mappoint->SetAttribute("ID", helper.GetMapPointID(this));
//
...
//    mappoint->SetAttribute("v3OneDownFromCenter_NC",
//                           helper.saveVector(v3OneDownFromCenter_NC).c_str());
//    mappoint->SetAttribute("v3OneRightFromCenter_NC",
//                           helper.saveVector(v3OneRightFromCenter_NC).c_str());
...
//    pPatchSourceKF = helper.GetKeyFrame(kfid);
...
//    v3OneDownFromCenter_NC = helper.loadVector<3>(
//            string(mappoint->FindAttribute("v3OneDownFromCenter_NC")->Value()));
//    v3OneRightFromCenter_NC = helper.loadVector<3>(
//            string(mappoint->FindAttribute("v3OneRightFromCenter_NC")->Value()));
...
//    v3PixelRight_W = helper.loadVector<3>(
//            string(mappoint->FindAttribute("v3PixelRight_W")->Value()));
//
...
//    }
//    else {
//        __android_log_print(ANDROID_LOG_INFO, "loading", "no pmmdata found!!!");
//    }
//
//}
