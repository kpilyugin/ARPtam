...
ATANCamera::ATANCamera(std::string sName, CVD::ImageRef imageSize) {
    ...
    if (useOpenCVDistortion) {
        ...
        opencv_camparams[3] = 2.4209867080666919e+02;
        opencv_camparams[4] = 1.8676740819913135e-01;
        opencv_camparams[5] = -6.0740481073990804e-01;
        opencv_camparams[6] = -1.0486813424120500e-04;
        opencv_camparams[7] = 7.4735485976131150e-04;
        opencv_camparams[8] = 4.3606770636640857e-01;
        cout << "OpenCV camera parameters are used. " << endl;
        cout << "First param = " << opencv_camparams[0] << endl;
    }
    RefreshParams();
}
...
void ATANCamera::RefreshParams() {
    ...
    // Work out the linear projection values for the UFB
    {
        ...
        if (useOpenCVDistortion) {
            ...
            vv2Verts.push_back(
                    UnProject(makeVector(mvImageSize[0] - 0.5, mvImageSize[1] / 2 - 0.5)));
            vv2Verts.push_back(
                    UnProject(makeVector(mvImageSize[0] / 2 - 0.5, mvImageSize[1] - 0.5)));
...
Vector<2> ATANCamera::Project(const Vector<2> &vCam) {
    ...
    if (useOpenCVDistortion) {
        ...
        mdLastFactor =
                1 + opencv_camparams[4] * r2 + opencv_camparams[5] * r4 + opencv_camparams[8] * r6;

        mvLastDistCam[0] = mdLastFactor * mvLastCam[0] +
                           2 * opencv_camparams[6] * mvLastCam[0] * mvLastCam[1] +
                           opencv_camparams[7] * (r2 + 2 * mvLastCam[0] * mvLastCam[0]);
...
Vector<2> ATANCamera::UnProject(const Vector<2> &v2Im) {
    ...
    if (useOpenCVDistortion) {
        ...
        for (int j = 0; j < iters; j++) {
            double r2 = x * x + y * y;
            double icdist = 1. / (1 + ((opencv_camparams[8] * r2 + opencv_camparams[5]) * r2 +
                                       opencv_camparams[4]) * r2);
            double deltaX =
                    2 * opencv_camparams[6] * x * y + opencv_camparams[7] * (r2 + 2 * x * x);
            double deltaY =
                    opencv_camparams[6] * (r2 + 2 * y * y) + 2 * opencv_camparams[7] * x * y;
...
Matrix<2, NUMTRACKERCAMPARAMETERS> ATANCamera::GetCameraParameterDerivs() {
    ...
    Vector<NUMTRACKERCAMPARAMETERS> vNNormal = mvCameraParams;
