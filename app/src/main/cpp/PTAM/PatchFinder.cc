...
PatchFinder::PatchFinder(int nPatchSize)
    ...
    int nMaxSSDPerPixel = 1000; // Pretty arbitrary... could make a GVar out of this. //default was 500!!!
...
int PatchFinder::CalcSearchLevelAndWarpMatrix(MapPoint& p,
    ...
    double pixelCamScale = 1; //compensate for different cameras FOV and resolutions
/*#ifdef __ANDROID__
  pixelCamScale = 6.9990767778699239e+002/5.3190712210112792e+002;
#endif*/
    // Project the source keyframe's one-pixel-right and one-pixel-down vectors into the current view
    Vector<3> v3MotionRight = se3CFromW.get_rotation() * p.v3PixelRight_W * pixelCamScale;
    Vector<3> v3MotionDown = se3CFromW.get_rotation() * p.v3PixelDown_W * pixelCamScale;
    // Calculate in-image derivatives of source image pixel motions:
    mm2WarpInverse.T()[0] = m2CamDerivs * (v3MotionRight.slice<0, 2>() -
                                           v3Cam.slice<0, 2>() * v3MotionRight[2] *
                                           dOneOverCameraZ) * dOneOverCameraZ;
    mm2WarpInverse.T()[1] = m2CamDerivs * (v3MotionDown.slice<0, 2>() -
                                           v3Cam.slice<0, 2>() * v3MotionDown[2] *
                                           dOneOverCameraZ) * dOneOverCameraZ;
...
#endif
#ifdef COMPUTE_NCC
    mnTemplateMean = ((float)mnTemplateSum)/(mnPatchSize*mnPatchSize);

    nSum = 0;
    nSumSq = 0;
    do
      {
        int b = mimTemplate[ir]-mnTemplateMean;
        //nSum += b;
        nSumSq +=b * b;
      }
    while(ir.next(mimTemplate.size()));
...
bool PatchFinder::FindPatchCoarse(ImageRef irPos, KeyFrame& kf, unsigned int nRange) {
...
#ifdef COMPUTE_NCC
    float nBestNCC = 0; // Best score so far is beyond the max allowed
   ...
   for(; i<i_end; i++)          // For each corner ...
     ...
     continue;              // ... reject all those not close enough..
       ...
       float nNCC;                // .. and find the NCC at those near enough.
       nNCC = NCCAtPoint(L.im, *i);
       if(nNCC > nBestNCC)      // Best yet?
     {
       irBest = *i;
       nBestNCC = nNCC;
     }
   ...
   if(nBestNCC > NCC_T)      // Found a valid match? //TODO make configurable??
     {
...
double PatchFinder::IterateSubPix(KeyFrame& kf) {
    ...
    for (ir.y = 1; ir.y < mnPatchSize - 1; ir.y++) {
        pTopLeftPixel = &im[PTAM::ir(v2Base) + ImageRef(1, ir.y)]; // n.b. the x=1 offset, as with y
        for (ir.x = 1; ir.x < mnPatchSize - 1; ir.x++) {
...
float PatchFinder::NCCAtPoint(CVD::BasicImage<CVD::byte>& im, const CVD::ImageRef& ir) {
    if (!im.in_image_with_border(ir, mirCenter[0]))
        return mnMaxSSD + 1;
    ...
    int nImageMean = ((float) nImageSum) / N;

    for (int nRow = 0; nRow < mnPatchSize; nRow++) {
        imagepointer = &im[irImgBase + ImageRef(0, nRow)];
        templatepointer = &mimTemplate[ImageRef(0, nRow)];
        for (int nCol = 0; nCol < mnPatchSize; nCol++) {
            int n = imagepointer[nCol] - nImageMean;
            //nImageSum += n;
            nImageSumSq += n * n;
            nCrossSum += n * (templatepointer[nCol] - mnTemplateMean);
        };
    }

    //cout << (((float)nCrossSum)/sqrt((float)nImageSumSq*mnTemplateSumSq)) << ", ";

    return ((float) nCrossSum) / sqrt((float) nImageSumSq * mnTemplateSumSq);
