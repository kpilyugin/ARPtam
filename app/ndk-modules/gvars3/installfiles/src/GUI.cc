...
namespace GVars3
{
  ...
  void GUI_impl::ParseLine(string s, bool bSilentFailure )
  {
	...
	{
		s = s.substr(strBegin, s.size() - strBegin);
	}

	int cmdpos = s.find_first_of(' ');
	if (cmdpos == std::string::npos)
		sCommand = s;
	else
	{
		sCommand = s.substr(0, cmdpos);

		sParams = s.substr(cmdpos, s.size() - cmdpos);

		const auto strBegin = sParams.find_first_not_of(' ');
		if (strBegin != std::string::npos)
		{
			sParams = sParams.substr(strBegin, s.size() - strBegin);
		}
	}
#else
	//int strsize = s.size();
